...
export function sortByStatusAndAlphabetically(collection: Restriction[]): Restriction[] {
  ...
  return [...collection].sort((restrictionA, restrictionB): number => {
    const statusSortResult =
      statuses.indexOf(restrictionA.status) - statuses.indexOf(restrictionB.status)

    if (statusSortResult !== 0) {
      return statusSortResult
    }

    return restrictionA.destinationLabel.localeCompare(restrictionB.destinationLabel)
  })
}
