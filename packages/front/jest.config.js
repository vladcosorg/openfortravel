...
module.exports = {
  globals: {
    ...
    'ts-jest': {
      tsconfig: '<rootDir>/packages/front/tsconfig.json',
      isolatedModules: true,
    },
  },
  setupFilesAfterEnv: ['<rootDir>/packages/front/test/jest/jest.setup.ts'],
  // noStackTrace: true,
  // bail: true,
  // cache: false,
  // verbose: true,
  ...
  coverageDirectory: '<rootDir>/packages/front/test/jest/coverage',
  collectCoverageFrom: [
    '<rootDir>/src/**/*.vue',
    '<rootDir>/src/**/*.js',
    '<rootDir>/src/**/*.ts',
  ...
  testMatch: [
    ...
    '<rootDir>/packages/front/test/jest/tests/**/*.(spec|test).+(ts|js)?(x)',
  ],
