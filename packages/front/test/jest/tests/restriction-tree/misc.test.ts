...
describe('Country sequence generation', () => {
  test('A list of countries should return a normal comma separated sequence', () => {
    expect(generateCountryAndAreaSequence(['fr', 'it', 'ro'], getCountryList())).toStrictEqual(
      'France, Italy and Romania',
    )
  })
  test('A list with a single item should return plain label', () => {
    expect(generateCountryAndAreaSequence(['fr'], getCountryList())).toStrictEqual('France')
  })
  ...
  test('A large list should get truncated', () => {
    expect(
      generateCountryAndAreaSequence(Object.keys(getCountryList()), getCountryList(), {
        compact: false,
      }),
    ).toStrictEqual(
