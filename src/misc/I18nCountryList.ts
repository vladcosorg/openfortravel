import { computed, watch, onServerPrefetch, toRef, ComputedRef } from '@vue/composition-api'
...
export const fetchCountryList = async (locale: string): Promise<I18nCountryList> => {
  ...
  translations = transform(translations, (list:I18nCountryList, label:string|string[], code: string) => {
...
export function useCountryList (): ComputedRef<{value: string, label:string}[]> {
  const store = useStore()
