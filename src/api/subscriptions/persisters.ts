...
export async function persistSubscriptionCollection(
  ...
  const { subscriptionCollection, firestore } = await import(
    'src/misc/firebase'
  )
...
export async function persistOriginSubscription(
  originCode: string,
  email: string,
) {
  const { subscriptionCollection, firestore, firebase } = await import(
    'src/misc/firebase'
  )
  const batch = firestore.batch()
